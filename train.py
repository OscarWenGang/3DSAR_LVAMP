# -*- coding: utf-8 -*-
"""
Created on Sat Jun 16 15:11:59 2018

Train the LVAMP network
...
#        nmse_  = tf.nn.l2_loss( (xhat_ - prob.x_)*maskX_) / nmse_denom_
        nmse_  = tf.nn.l2_loss( (xhat_ - prob.x_)*maskX_)
...
def do_training(training_stages,prob,savefile,ivl=20,maxit=100000,better_wait=5000):   # maxit=1000000,better_wait=5000
    """
    ...
    for name,xhat_,loss_,nmse_,train_,var_list in training_stages:
        ...
        for i in range(maxit+1):
            if i%ivl == 0:
                ...
                sys.stdout.flush()
                if i%(100*ivl) == 0:
                    s = sess.run(merged_summary, feed_dict={prob.y_:prob.yval,prob.x_:prob.xval})
                    writer.add_summary(s, it + i/ivl/100)
